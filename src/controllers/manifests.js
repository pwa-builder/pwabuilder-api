...
exports.create = function (client, storage, pwabuilder, raygun) {
  ...
  return {
    ...
    create: function (req, res, next) {
      if (req.body.siteUrl) {
        pwabuilder
          .createManifestFromUrl(req.body.siteUrl, client)
        ...
        pwabuilder
          .createManifestFromFile(file, client)
          .then(function (manifest) {
            res.json(manifest);
          })
          .fail(function (err) {
    ...
    update: function (req, res, next) {
      pwabuilder
        .updateManifest(client, req.params.id, req.body)
        .then(function (manifest) {
          res.json(manifest);
        })
        .fail(function (err) {
      ...
      client.get(req.params.id, function (err, reply) {
        if (err) {
          ...
          return res.status(500).json({
            error:
              'There was a problem loading the project, please try building it again.',
          });
        }
        if (!reply) return res.status(404).send('NOT FOUND');
        ...
        storage
          ...
          .then(function (normManifest) {
            ...
            return pwabuilder.createProject(
              manifest,
              output,
              platforms,
              req.query.href || '',
          ...
          .then(function (projectDir) {
            if (req.query.ids) {
              ...
              return pwabuilder
                .getServiceWorkers(req.query.ids)
          ...
          .then(function () {
            return storage.removeDir(output);
          })
          .then(function () {
      ...
      client.get(req.params.id, function (err, reply) {
        ...
        storage
          ...
          .then(function (err) {
            ...
            return Q.nfcall(
              fs.readFile,
              path.join(projectDirectory, 'windows10', 'test_install.ps1')
            );
          })
          .then(function (data) {
          ...
          .then(function (err) {
            ...
            return Q.nfcall(
              fs.copy,
              path.join('bin', 'assets', 'readme.md'),
              path.join(projectDirectory, 'windows10', 'readme.md')
            );
          })
          .then(function (err) {
            // Manifest file is now ready to be processed by packager
            return pwa10.package(projectDirectory, {
              DotWeb: false,
              AutoPublish: false,
              Sign: false,
            });
    ...
    package: function (req, res) {
      client.get(req.params.id, function (err, reply) {
        if (err) {
          ...
          return res.status(500).json({
            error:
              'There was a problem packaging the project, please try packaging it again.',
          });
        }
        if (!reply) return res.status(404).send('NOT FOUND');
        ...
        if (!platform) {
          ...
          return res
            .status(400)
            .json({ error: 'No platform has been provided' });
        }
        ...
        var result = storage
          ...
          .then(function () {
            return pwabuilder.addScreenshots(manifest);
          })
          .then(function () {
            return pwabuilder.normalize(manifest);
          })
          ...
          .then(function (projectDir) {
            var paths = [
              ...
              $ = cheerio.load(win10manifest, {
                xmlMode: true,
                decodeEntities: false,
              });

            $('Package Identity').attr(
              'Publisher',
              'CN=CD81B1A7-2407-491F-ACA2-03B1F7A0F020'
            );
            $('Package Properties PublisherDisplayName').text(
              'Progressive Apps Indexer'
            );
          ...
          .then(function (projectDir) {
            return pwabuilder.packageProject(
              platforms,
              projectDir,
              packageOptions
            );
            ...
            .then(function (packagePaths) {
              if (packagePaths.length > 1) {
                return res.status(400).json({
                  error: 'Multiple packages created. Expected just one.',
                });
              }
    ...
    generateMissingScreenshots: function (req, res) {
      //console.log(req.params.id);
      client.get(req.params.id, function (err, reply) {
        if (err) {
          return res.status(500).json({
            error:
              'There was a problem loading the manifest, please try it again.',
          });
        }
        if (!reply) return res.status(404).send('NOT FOUND');
        var manifestInfo = JSON.parse(reply);

        var screenshots = req.body;
        if (manifestInfo.content.screenshots == undefined) {
          manifestInfo.content.screenshots = [];
        }
        var assets = [];
        for (var i = 0; i < screenshots.length; i++) {
          //console.log('Source:', screenshots[i].src.substr(0, 4));
          var screenshotInfo = {
            src:
          ...
          //Push to assets folder
          assets.push({
            fileName: 'screenshot' + (i + 1).toString() + '.png',
            data: hexString,
          });
          //Push screenshot data to manifest
          manifestInfo.content.screenshots.push(screenshotInfo);
        }

        pwabuilder
          .updateManifest(client, req.params.id, manifestInfo.content, assets)
    ...
    generateMissingImages: function (req, res) {
      client.get(req.params.id, function (err, reply) {
        ...
        var manifestInfo = JSON.parse(reply);
        manifestInfo.content.icons = manifestInfo.content.icons.filter(
          function (icon) {
            return !icon.generated;
          }
        );
        var persistedIcons = JSON.parse(
          JSON.stringify(manifestInfo.content.icons)
        );

        var imageFile = req.files[0];
        Q.nfcall(fs.readFile, imageFile.path)
          .then(function (imageContents) {
            pwabuilder
              .generateImagesForManifest(imageContents, manifestInfo, client)
              .then(function (manifest) {
                ...
                var assets = [
                  {
                    ...
                    data: imageContents.toString('hex'),
                  },
                ];
                return pwabuilder.updateManifest(
                  client,
                  req.params.id,
                  manifest,
                  assets
